  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (project.isFunded) {
        ...
        await contract.deleteProject(project.id.toNumber());
              ...
              {isCreator && (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg ${
                    project.isFunded && !project.isExpired
                      ? 'bg-red-600 hover:bg-red-700 text-white'
                      : 'bg-purple-600 hover:bg-purple-700 text-white'
                  }`}
                >
                  {project.isFunded && !project.isExpired ? 'Withdraw Funds' : 'Delete Project'}
                </button>
              )}
            </div>
                  ...
                  {backers && backers.length > 0 ? (
                    backers.map((backer, index) => (
                      <div key={index} className="flex justify-between text-sm text-gray-300">
                        <span>{backer.address?.slice(0, 6)}...{backer.address?.slice(-4)}</span>
                        <span>{formatEther(backer.amount)} ETH</span>
                      </div>
                    ))
                  ) : (
                    <p className="text-gray-400 text-sm">No backers yet</p>
                  )}
                </div>
              </div>
            </div>
